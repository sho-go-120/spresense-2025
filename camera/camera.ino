...
// stream functionはカメラのプレビュー画像を取得
// setting functionはカメラのパラメータを設定
// capture picture functionは高解像度のJPEG画像を取得
...
void CamCB(CamImage img) // フレームを取得された時のコールバック関数
{
...
void setup()
{
  ...
  err = theCamera.begin(); // theCameraをbegin
  ...
  err = theCamera.setStillPictureImageFormat( // 画像のパラメータをset
     CAM_IMGSIZE_QUADVGA_H,
     CAM_IMGSIZE_QUADVGA_V,  // QVGA 320×240
     CAM_IMAGE_PIX_FMT_JPG); // format JPG
  if (err != CAM_ERR_SUCCESS)
    {
      printError(err);
    }
  ...
  if (take_picture_count < TOTAL_PICTURE_COUNT)
    {
      ...
      */
      ...
      CamImage img = theCamera.takePicture(); // カメラデータを取得
      ...
      if (img.isAvailable())
        {
          ...
          myFile.write(img.getImgBuff(), img.getImgSize()); // SDカード書き込み
          myFile.close();
        }
      else
        {
