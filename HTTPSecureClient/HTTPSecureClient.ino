...
void setup() {
	...
	File rootCertsFile = theSD.open(ROOTCA_FILE, FILE_READ); // ルート証明書をget
...
void loop() {
	...
	while (1) {
		...
		case POST:
			...
			result = theHttpGs2200.post(HTTP_POST_PATH, sendData); // POSTを送信
			if (false == result) {
				break;
			}

			do { // POST要求に対し、なんらかの受信データがある場合にReceiveによる受信を行う
